...
const MockWalletConnect = ({ onConnect }: { onConnect: (wallet: any) => void }) => {
  ...
  return (
    ...
    >
      {mockWallet.isConnected ? `Connected: ${mockWallet.address?.slice(0, 6)}...` : 'Connect Wallet'}
    </button>
  )
}
  ...
  return (
    <div data-testid="swap-review">
      ...
      <div data-testid="swap-details">
        <div>From: {fromAmount} {fromToken?.symbol}</div>
        <div>To: {toAmount} {toToken?.symbol}</div>
        <div>Slippage: {slippage}%</div>
        <div>Minimum Received: {minimumReceived} {toToken?.symbol}</div>
      </div>
    ...
    return (
      <div data-testid="swap-success">
        ...
        <div data-testid="success-details">
          ...
          <div>Swapped: {swapResult.fromAmount} {swapResult.fromToken} → {swapResult.toAmount} {swapResult.toToken}</div>
        </div>
        <button 
...
describe('End-to-End Swap Flow Integration Tests', () => {
  ...
  describe('Complete Swap Flow', () => {
    it('completes full swap flow: wallet connect → token select → amount input → review → execute', async () => {
      ...
      await waitFor(() => {
        ...
        expect(screen.getByTestId('success-details')).toHaveTextContent('Swapped: 1 ETH → 2000.000000 USDC')
        expect(screen.getByTestId('success-details')).toHaveTextContent('Transaction Hash: 0xabcdef')
      }, { timeout: 3000 })
    })
  ...
  describe('Error Scenarios', () => {
    it('handles network errors during swap execution', async () => {
      ...
      await waitFor(() => {
        expect(screen.getByTestId('error-message')).toHaveTextContent('Swap failed. Please try again.')
      }, { timeout: 3000 })
    })
  })
})