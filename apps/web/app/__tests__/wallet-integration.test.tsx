...
const connectWallet = async (connector?: string): Promise<{ address: string; balance: string; chainId?: string }> => {
...
const addToken = async (tokenAddress: string, tokenSymbol: string, tokenDecimals: number, connector?: string): Promise<boolean> => {
  if (connector === 'metaMask') {
    const globalWindow = (global as any).window
    if (globalWindow?.ethereum) {
...
describe('Wallet Integration Tests', () => {
  ...
  describe('MetaMask Integration', () => {
    ...
    it('should refresh balance via MetaMask', async () => {
      ...
      const balance = await refreshBalance('0x1234567890123456789012345678901234567890', 'metaMask')
  ...
  describe('WalletConnect Integration', () => {
    ...
    it('should throw error when WalletConnect has no accounts', async () => {
      ...
      await expect(connectWallet('walletConnect')).rejects.toThrow('WalletConnect: No accounts found')
    })
    ...
    it('should refresh balance via WalletConnect', async () => {
      ...
      const balance = await refreshBalance('0x9876543210987654321098765432109876543210', 'walletConnect')
  ...
  describe('Error Handling', () => {
    ...
    it('should handle token addition rejection', async () => {
      ...
      const result = await addToken('0xA0b86a33E6441c8C06DD2b7c94b7E6E6E6E6E6E6E6', 'USDC', 6, 'metaMask')
