...
**Phase 1 security status (2026-07-25):** authenticated sessions reject malformed, expired, future-dated, or cross-chain records, and authentication failures are sanitized at the HTTP boundary. SIWE nonces now use one Durable Object per nonce with serialized consume, so concurrent verification can mint at most one session from a nonce. The remaining production gate is deployment configuration and live end-to-end validation of the Durable Object binding.
