...
- **[G2.5] Make the TWAP real (pre-automation gate)**: `AetherHook` already records a 1024-slot observation buffer, but it has **two defects**. (1) `getCurrentTwap` returns a cumulative delta over *sample counts*, not a *time-weighted* average. (2) More fundamentally, each sample today is derived in `afterSwap` from `amountIn / amountOut` — the swap's **volume-dependent average execution price, not the pool's terminal spot price**; after a large cross-tick trade, holding that value until the next observation yields an incorrect TWAP *even once it is time-weighted*. **Fix:** stop sampling from execution amounts — instead record the pool's **spot state at observation time** (the normalized **tick / `sqrtPriceX96`** read from `PoolManager`/`StateView`) and accumulate *that* over **elapsed time** (a Uniswap-`observe()`-style time-weighted-tick accumulator), converting the time-weighted tick to price; also **normalize pair direction** (reciprocal for reverse-direction pairs). Add time-weighted + **cross-tick** + fuzz tests — **before** any Phase-2 TP/SL/auto-recenter logic relies on it.
...
**Phase 1 implementation status (2026-07-24):** the first contract increment now adds an atomic V4 swap-plus-liquidity zap with deadline, swap slippage, minimum liquidity-consumption, dust-refund, and reentrancy guards. Existing router positions also have an owner ledger that blocks arbitrary public removal. A standalone `AetherPositionManager` receipt-NFT foundation is implemented, tested, and included in the Sepolia deployment script as the canonical transferable-position artifact; router legacy-ledger positions remain a compatibility surface until the ownership migration is complete. The web surface now includes protected liquidity/range, rebalance-intent, pool-creation-intent, and portfolio states. Pool creation has a deadline-enforced factory entrypoint, but wallet submission remains gated until a real live price guard and native manager/router calldata are available. The ledger is a security containment layer, not a substitute for native V4 PositionManager integration.
...
> **MEV ordering note (two tiers — protected submission precedes Phase 1).** Slippage bounds + deadlines (Tier A) only **cap** how much a public-mempool sandwich can extract; they do **not prevent** an attacker from front-running + back-running a zap/rebalance *within the user's own tolerance*. Read the control in two tiers, with different phase preconditions:
> - **Tier A — value caps (present from Phase 0).** `AetherRouter` enforces **slippage bounds + transaction deadlines** (plus SafeERC20 + reentrancy guard — §6.3). These **bound, but do not eliminate,** sandwich loss.
> - **Tier B — sandwich mitigation for *user-signed* flows (must precede Phase 1).** Phase-1 **zaps / one-click rebalances are user-signed fund-moving transactions** broadcast to the public mempool, so **protected/private transaction submission** (Flashbots Protect / MEV-Blocker-style private RPC, or commit-reveal / batched inclusion) **must be wired into the Phase-1 zap/rebalance transaction-building path before Phase 1 ships** — Tier A alone leaves them sandwichable within tolerance. **Do not defer this to Phase 3.**
> - **Tier B for *unattended* automation (Phase-3 full suite, still a priority gate).** The **Phase-2 keeper's funded signer/relayer** (§9 Phase 2) also submits fund-moving transactions; the **full MEV suite** (protected/private submission at the relayer layer) **plus the Phase-3 indexer** (for at-scale on-chain verification) must land **before the keeper runs unattended**. Until then, Phase-2 automation stays **reconciliation-gated per action**.
> Net: no fund-moving flow runs without its required MEV tier — user-signed Phase-1 flows get Tier A (Phase 0) **and Tier B private submission (pre-Phase-1)**; unattended Phase-2 flows are gated on **Tier B at the relayer layer** and the **Phase-3 indexer**.
