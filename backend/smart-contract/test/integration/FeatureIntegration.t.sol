...
contract FeatureIntegrationTest is Test {
    ...
    function _setupChainInfrastructure(uint16 chainId) internal {
        ...
        // Set the hook address for this manager
        manager.setHookAddress(address(dynamicFeeHook)); // This updates the state of the contract at address(manager)

        // Register the pool for dynamic fees in the FeeRegistry, authorizing the hook
        // The PoolKey used here (keyForRegistration) already has its .hooks member set to address(dynamicFeeHook)
    ...
    function test_DynamicFeeAdjustment() public {
        uint16 chainId = networks.ETHEREUM_CHAIN_ID(); // Get Ethereum chain ID
        MockPoolManager manager = poolManagers[chainId];
        require(address(manager) != address(0), "Manager not found for dynamic fee test");

        // Initial fee check
        PoolKey memory key = PoolKey({
            token0: token0Addresses[chainId],
            token1: token1Addresses[chainId],
            fee: DEFAULT_FEE,
    ...
    function _performSwap(uint16 chainId, bool zeroForOne, uint256 amount) internal {
        ...
        BalanceDelta memory delta = manager.swap(key, params, ""); // No hook data needed for this mock

        uint256 amountOut = uint256(int256(delta.amount1) * -1); // amount1 is negative for zeroForOne swap
        assertTrue(amountOut > 0, "Output amount should be greater than zero");
        ...
        assertEq(balanceAfter - balanceBefore, amountOut, "Recipient balance should increase by the output amount");
    }
