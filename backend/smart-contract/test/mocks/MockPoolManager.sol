...
contract MockPoolManager is IPoolManager {
    // Custom Errors
    error MPM_InvalidPoolAddress();
    error MPM_InvalidSwapAmount();
    error MPM_PoolNotSetForId();
    error MPM_PoolHasNoBytecode(); // Used for general key/pool config errors if specific token/fee errors are not defined
    error MPM_HookAddressNotSet();
    error MPM_InvalidHookSelector();
    error MPM_HookCallFailed(string reason);
    error MPM_LiquidityDeltaZero();
    error MPM_OperationNotImplemented();
    error MPM_AmountExceedsUint128Max();
    error MPM_PoolAlreadyInitialized();
    error MPM_PoolNotFound();
    error MPM_HookPermissionDenied();
    ...
    function setPool(bytes32 poolId, address _poolAddress) external {
        ...
        if (_poolAddress == address(0)) revert MPM_InvalidPoolAddress();
        ...
        returns (BalanceDelta memory delta)
    {
        ...
        if (amountIn == 0) revert MPM_InvalidSwapAmount();
    ...
    function _validateKey(PoolKey calldata key) internal view returns (address poolAddress) {
        ...
        if (poolAddress == address(0)) revert MPM_PoolNotSetForId();
        if (address(poolAddress).code.length == 0) revert MPM_PoolHasNoBytecode();
        ...
        if (key.token0 != poolToken0) revert MPM_PoolHasNoBytecode();
        if (key.token1 != poolToken1) revert MPM_PoolHasNoBytecode();
        // Hook validation remains the same, assuming hookAddress is global for the mock
        if (key.hooks != hookAddress) {
            revert MPM_HookAddressNotSet();
        }
    }
        ...
        internal
    {
        if (hookAddress != address(0)) {
            if (!Hooks.hasPermission(hookAddress, Hooks.BEFORE_SWAP_FLAG)) revert MPM_HookPermissionDenied();
            try BaseHook(hookAddress).beforeSwap(msg.sender, key, params, hookData) returns (bytes4 selector) {
                if (selector != BaseHook.beforeSwap.selector) revert MPM_InvalidHookSelector();
                emit SwapHookCalled(true, msg.sender, BalanceDelta(0, 0));
            } catch Error(string memory reason) {
                emit HookError(reason);
                revert MPM_HookCallFailed(reason);
            } catch {
                revert MPM_HookCallFailed("Unknown hook error");
            }
        }
    }
    ...
    function _handleAfterSwapHook(
        ...
        if (hookAddress != address(0)) {
            if (!Hooks.hasPermission(hookAddress, Hooks.AFTER_SWAP_FLAG)) revert MPM_HookPermissionDenied();
            try BaseHook(hookAddress).afterSwap(msg.sender, key, params, delta, hookData) returns (bytes4 selector) {
                if (selector != BaseHook.afterSwap.selector) revert MPM_InvalidHookSelector();
    ...
    function modifyPosition(
        ...
        if (params.liquidityDelta > 0) {
            ...
            if (liquidityToAdd == 0) revert MPM_LiquidityDeltaZero();
            ...
            if (requiredAmount0 == 0 && requiredAmount1 == 0) revert MPM_PoolHasNoBytecode();
            ...
            if (liquidityToRemove == 0) revert MPM_LiquidityDeltaZero(); // Sanity check
    ...
    function initialize(PoolKey calldata key, uint160 sqrtPriceX96, bytes calldata hookData) external override {
        ...
        bytes32 poolId = keccak256(abi.encode(key));
        if (initializedPools[poolId]) revert MPM_PoolAlreadyInitialized();
        ...
        if (key.hooks != address(0) && Hooks.hasPermission(key.hooks, Hooks.AFTER_INITIALIZE_FLAG)) {
            ...
            try BaseHook(key.hooks).afterInitialize(msg.sender, key, sqrtPriceX96, hookData) returns (bytes4 selector) {
                if (selector != BaseHook.afterInitialize.selector) revert MPM_InvalidHookSelector();
            } catch Error(string memory reason) {
                // Handle or log hook error if necessary for testing
                revert MPM_HookCallFailed(reason);
            } catch {
                revert MPM_HookCallFailed("Unknown hook error during afterInitialize");
            }
        }
    ...
    function getPool(PoolKey memory key) external view override returns (address poolAddress) {
        bytes32 poolId = keccak256(abi.encode(key));
        poolAddress = pools[poolId];
        if (poolAddress == address(0)) revert MPM_PoolNotFound();
    }
}
