...
interface IAetherPool {
    ...
    /// @param feeRate The fee paid for the swap.
    event Swap(
    ...
    function initialize(address token0, address token1, uint24 feeRate) external;
    ...
    /// @return feeRate The current pool fee.
    function fee() external view returns (uint24 feeRate);
}
