...
contract RoleManager is AccessControlEnumerable, ReentrancyGuard {
     ...
     */
    function grantTimedRole(bytes32 role, address account, uint256 duration) external validRole(role) nonReentrant {
     ...
     */
    function proposeRoleChange(bytes32 role, address account, bool isGrant)
        external
        validRole(role)
     ...
     * @param nonce The nonce of the role change proposal
     */
    function executeRoleChange(bytes32 role, address account, bool isGrant, bytes32 nonce) external validRole(role) {
