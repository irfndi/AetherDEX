...
contract AetherRouter is ReentrancyGuard, Ownable, Pausable {
    ...
    event PoolRegistered(bytes32 indexed poolId, address indexed token0, address indexed token1, uint24 fee);
    ...
    constructor(IPoolManager _poolManager, address _feeRecipient, address _initialOwner) Ownable(_initialOwner) {
        ...
        returns (uint256 amountOut)
    {
        ...
        emit SwapExecuted(
            params.tokenIn, params.tokenOut, params.amountIn, amountOut, params.recipient, _encodePath(optimalPath)
        );
    }
        ...
        returns (uint256 amountIn)
    {
        ...
        emit SwapExecuted(
            params.tokenIn, params.tokenOut, amountIn, params.amountOut, params.recipient, _encodePath(optimalPath)
        );
    }
     ...
     * @return amountOut Amount of tokens received
     */
    function exactInputMultiHop(bytes calldata path, uint256 amountIn, uint256 amountOutMinimum, address recipient)
        external
        payable
        nonReentrant
        whenNotPaused
        returns (uint256 amountOut)
    {
        ...
        emit SwapExecuted(
            pathInfo.tokens[0], pathInfo.tokens[pathInfo.tokens.length - 1], amountIn, amountOut, recipient, path
        );
    }
     ...
     */
    function registerPool(PoolKey calldata key, uint256 liquidity) external onlyOwner {
        bytes32 poolId = keccak256(abi.encode(key.currency0, key.currency1, key.fee, key.tickSpacing, key.hooks));

        pools[poolId] =
            PoolInfo({key: key, active: true, liquidity: liquidity, fee: key.fee, lastUpdate: uint32(block.timestamp)});
     ...
     */
    function updatePoolLiquidity(bytes32 poolId, uint256 liquidity) external onlyOwner {
     ...
     */
    function _findOptimalPath(address tokenIn, address tokenOut, uint256 amount, bool exactInput)
        internal
        view
        returns (PathInfo memory)
    {
     ...
     */
    function _findMultiplePaths(address tokenIn, address tokenOut, uint256 amountIn)
        internal
        view
        returns (PathInfo[] memory)
    {
        ...
        for (uint256 i = 0; i < intermediateTokens.length && pathCount < maxPathsToCheck; i++) {
            ...
            if (pathCount < maxPathsToCheck - 1) {
                for (uint256 j = i + 1; j < intermediateTokens.length && pathCount < maxPathsToCheck; j++) {
                    ...
                    PathInfo memory threeHopPath =
                        _findThreeHopPath(tokenIn, intermediate, intermediate2, tokenOut, amountIn);
     ...
     */
    function _findDirectPath(address tokenIn, address tokenOut, uint256 amountIn)
        internal
        view
        returns (PathInfo memory path)
    {
     ...
     */
    function _findTwoHopPath(address tokenIn, address intermediate, address tokenOut, uint256 amountIn)
        internal
        view
        returns (PathInfo memory path)
    {
        ...
        if (pool1.currency0 == Currency.wrap(address(0)) || pool2.currency0 == Currency.wrap(address(0))) {
    ...
    function _findThreeHopPath(
        ...
        if (
            pool1.currency0 == Currency.wrap(address(0)) || pool2.currency0 == Currency.wrap(address(0))
    ...
    function _executeSwap(
        ...
        uint256 finalAmount = _executeMultiHopSwap(path, amountIn, amountOutMin, recipient, exactInput);
    ...
    function _executeMultiHopSwap(
        ...
        for (uint256 i = 0; i < path.pools.length; i++) {
            ...
            // Execute single swap
            currentAmount =
                _executeSingleSwap(path.pools[i], path.tokens[i], path.tokens[i + 1], currentAmount, currentRecipient);
        }
    ...
    function _executeSingleSwap(
        ...
        uint256 amountOut = zeroForOne ? uint256(-delta.amount1) : uint256(-delta.amount0);
     ...
     */
    function _findBestPool(address tokenA, address tokenB) internal view returns (PoolKey memory) {
        ...
        for (uint256 i = 0; i < feeTiers.length; i++) {
            ...
            if (poolInfo.active && poolInfo.liquidity > bestLiquidity && poolInfo.liquidity >= minLiquidityThreshold) {
     ...
     */
    function _estimateSwapOutput(PoolKey memory poolKey, address tokenIn, uint256 amountIn)
        internal
        view
     ...
     */
    function _performSwapWithLowLevelCall(PoolKey memory poolKey, IPoolManager.SwapParams memory swapParams)
        internal
        returns (BalanceDelta memory delta)
    {
        // Call the pool manager with proper calldata types
        (bool success, bytes memory result) =
            address(poolManager).call(abi.encodeWithSelector(IPoolManager.swap.selector, poolKey, swapParams, ""));
